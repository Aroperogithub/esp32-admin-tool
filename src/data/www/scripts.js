...
const formulario = document.getElementById('form');         //Este 'form' se declara como "id" en el "mqtt.html"
const inputs = document.querySelectorAll('#form input');

// Objeto de las expresiones Regulares.
const expresiones = {
    //TextNumber: /^[a-zA-Z0-9]{4,30}$/,                      // Letras a-z A-Z minúsculas y mayúsculas, números caracteres de 4 mínimo a 30 dígitos máximo
    TextNumber: /^[a-zA-Z0-9\@\.]{4,30}$/,                      // Letras a-z A-Z minúsculas y mayúsculas, números, la "@" y el ".", caracteres de 4 mínimo a 30 dígitos máximo
    TextUnderscore: /^[a-zA-Z0-9\_]{4,30}$/,                    // Letras minúsculas - mayusculas, números y guión bajo de 4 mínimo a 30 digitos máximo.
    ...
    Dominio: /^([a-zA-Z0-9-]{1,63}(\.[a-zA-Z0-9-]{1,63})*)$/,    //Esta expresión regular valida un dominio que puede tener uno o más subdominios separados por puntos. 
                                                                // Cada subdominio puede tener entre 1 y 63 caracteres alfanuméricos y guiones
...
// Dejar de teclear y la pérdida del foco del input.
inputs.forEach((input) => {                                 //Función flecha.
    input.addEventListener('keyup', validarFormulario);     //Captura el evento de un click.
    input.addEventListener('blur', validarFormulario);      //Captura el evento cuando pierde el foco.
});
// Función para validar el forumario
function validarFormulario (e) {
    //console.log (e.target.dataset.expresion);
    switch (e.target.name) {
        // SECCIÓN MQTT
        case "mqtt_id":
            validarCampo (expresiones[e.target.dataset.expresion], e.target, e.target.name); // El e.target envía todo el input completo.
            break;
        case "mqtt_user":
...
const validarCampo = (expresion, input, campo) => {
    //console.log (campo);
    if (expresion.test(input.value) && input.value != '') {     //Recordar que con el RegExp ".test" se comprueba la propiedad 'value' que se le pasa a esa función. Además se comprueba que no viene vacío.
...
function manejadorValidacion(e) {
    e.preventDefault();                 // Esto detiene completamente el evento.
    let page = document.getElementById('page').innerHTML;
    if (page == "Configuración del Broker MQTT") {
        //console.log(page);                        //Esto es para validar que me llega: "Configuración del Broker MQTT"
...
// Escaneo de redes
let escanear = () => {
    const url = 'http://' + location.hostname + '/scan';        //Este es el URL que creamos como acceso al acceso al Escaneo
    fetch(url)
        .then(respuesta => respuesta.json())                    //Este espera una respuesta que es un JSON.
        .then(data => {                                         //Este es para manejar la respuesta, que es lo que responde el Dispositivo.
            if (pathname == "/esp-wifi") {                      //Solo se va a poder realizar desde el módulo WiFi.
                let redes = document.getElementById('redes');   // capturamos el id de redes
                let serial = document.getElementById('serial'); // capturamos el id de serial
                redes.innerHTML = "";                           //limpiamos redes
                serial.innerHTML = "";                          //limpiamos serial
                console.log(data);                              // mostrar json por consola
                if (data.code == 1) {                           // Mensaje de retorno Respuesta del Scan de redes WIFI
                    // Count > 0
                    let mitabla = document.getElementById('mi_tabla');  //capturamos el id del Body para dibujar la tabla
                    mitabla.innerHTML = "";                     //limpiamos tabla
                    redes.innerHTML = data.meta.count;          // Agregamos cantidad de redes
                    serial.innerHTML = "<i class='icon-barcode'></i> " + data.meta.serial;  // Agregamos el serial del dispositivo
                    for (let valor of data.data) {              // Recorremos toda la lonjitud del data key DATA
