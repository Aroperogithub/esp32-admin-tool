...
void handleHome(AsyncWebServerRequest *request) {
    ...
    if (file){
        ...
        s.replace(F("#mqtt_server#"), F(mqtt_server));
...
void InitServer(){
    ...
    server.serveStatic("/www/safepool.png", SPIFFS, "/www/safepool.png").setDefaultFile("www/safepool.png").setCacheControl("max-age=600");
    ...
    server.serveStatic("/www/Antidesbordamiento.png", SPIFFS, "/www/Antidesbordamiento.png").setDefaultFile("www/Antidesbordamiento.png").setCacheControl("max-age=600");
    ...
    server.on("/esp-admin", HTTP_POST, [](AsyncWebServerRequest *request){
        ...
        if(request->params() == 5) {                //Son 5 parámetros: username, password, new_username, new_password, confirmación de new_password.
            String u, p, nu, np, cp;                //Variables auxiliares para los 5 parámetros que nos llegan.
            ...
            u.trim();                               //Quita todos los espacios a todo lo que está llegando en el String.
            p.trim();
            nu.trim();
            np.trim();
            cp.trim();
            //log ("User: " + u + "Pass: " + p + nu + np + cp);
        ...
        if (request->params() == 1) {
            ...
            if (s != "" && s != id) {
                ...
                if(settingsSaveWiFi()){
                    ...
                    request->send(200, "text/html", SweetAlert("Identificador", "Error", "El Identificador no se pudo Guardar en SPIFFS", "error", "aviso")); 
                    return;
                }              
            } else {
    ...
    server.on("/esp-mqtt", HTTP_POST, [](AsyncWebServerRequest *request){
        ...
        if (mqtt_enable == 1) {                         //Esto es porque si el mqtten == "1" enviará todos los valores de las 6 variables + 1 del mqtten y deben guardarse todos ellos.
            if (request->params() == 7){
                String s;
                // ID MQTT Broker
        ...
        } else {            //En este caso en el que mqtten == "0", entonces solo se guarda una variable, en este caso la de mqtten.
            // leds
            leds();
            // Guardar la configuracion del mqtt
            if(settingsSaveMQTT()){
    ...
    server.on("/esp-wifi", HTTP_POST, [](AsyncWebServerRequest *request){
        ...
        if (settingsSaveWiFi()){
            ...
            return;
        } else {    //Aquí solo va a entrar si no se ha podido cargar en el SPIFFS (Sería algo raro que salga).
